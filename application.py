...
def experiments():
    white_list= ['intact_whitenoise_1', 'ambient_1hr']
    ...
    return Response(json.dumps(white_listed), status = 200)
...
def form_query(specs):
    ...
    starting_seq = int(specs['min_sequence']) + 1000*int(specs['userstart'])
    ending_seq = int(specs['min_sequence']) + 1000*int(specs['userfinish'])
